...
float h, t, p, f, f_mean, b, p_mattino, h_mattino;
double f_media;
double d_double;
...
RTC_DATA_ATTR static time_t food, candito;
  ...
  while (t_double<nanna){
    if (millis() > lastTimeBotRan + botRequestDelay)  {
      ...
      if (d_double>2 && d_double<7){
        ...
        //miele
        if (p>(p_mattino+1000) && h<h_mattino){
          numNewMessages=1;
        }
      }
      // check batteria
      ...
      //inverno, check del cibo
      if (d_double>10 || d_double<3){ 
        if (p=food-candito){
          numNewMessages=1;
        }
      }
...
void handleNewMessages(int numNewMessages) {
  ...
  for (int i=0; i<numNewMessages; i++) {
    ...
    if (d_double>2 && d_double<7){
      ...
      }else if (p>(p_mattino+1000) && h<h_mattino){
          String miele= "The weigth increase of 1Kg, bees are doing honey";
          bot.sendMessage(chat_id, miele, "");
      }
    }else if (battery_text==19){
        String bm ="Battery is at 15%, please check and replace it. ";
        bot.sendMessage(chat_id, bm, "");
    }else if (d_double>10 || d_double<3){
      if (p=food-candito){
          String cibo= "Candy is finisheg, please give food to your bees.";
          bot.sendMessage(chat_id, cibo, "");
      }
    }else if (f_media>600){
        String orfane ="Queen is dead. The bees are orphans";
        bot.sendMessage(chat_id, orfane, "");
    }else{
      ...
      if (text == "/start") {
        ...
        if (d_double>10 || d_double<3){
          welcome += "/candy if you give you bees food \n";
        }
        ...
        String inverno= "Write the weight of the food you give the bees in grams";
        bot.sendMessage(chat_id, inverno, ""); 
        String text_inverno = bot.messages[i].text;
        double cand = text_inverno.toDouble();
        candito = cand;
        food= p_mattino;
      } else {
